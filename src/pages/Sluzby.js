import React from "react"

import img1 from "../images/SluzbyImgs/Sluzby1.jpg"
import img2 from "../images/SluzbyImgs/Sluzby2.jpg"
import img3 from "../images/SluzbyImgs/Sluzby3.jpg"
import img4 from "../images/SluzbyImgs/Sluzby4.jpg"
import img5 from "../images/SluzbyImgs/Sluzby5.webp"
import img6 from "../images/SluzbyImgs/Sluzby6.jpg"
import img7 from "../images/SluzbyImgs/Sluzby7.jpg"
import img8 from "../images/SluzbyImgs/Sluzby8.jpg"

export default function Sluzby(){
    return (
        <div className="full--sluzby">
{/* segment ------------------------------------------------*/}
            <div className="uvod">
                <div className="uvod--nadpis">
                    <h1>BÝVANIE</h1>
                </div>
                <div className="uvod--text">
                    <p>Vyriešiť otázku vlastného bývania je cieľom mnohých mladých ľudí, avšak nie len ich. Viete v akej hodnote môžete hľadať svoje vysnívané bývanie, aby ste neprekročili hodnotu zdravého zadlženia sa? Ako vyzerá hypotéka v číslach? Máte záujem o zníženie celkovej preplatenosti? Rád Vám s tým pomôžem a pripravím nezáväzný plán na základe Vašej konkrétnej situácie.</p>
                </div>
            </div>
            
            <div className="box">
                    <div className="boxText">
                        <h1>Prvé bývanie</h1>
                        <hr className="hr--podNadpisom" />
                        <p>Prvé vlastné bývanie je veľkým krokom v živote. Pre tých, ktorí to berú na vlastné ramená a chcú pri tom využiť peniaze banky, odporúčam dôsledne sa pripraviť. Častokrát sa stretávam s otázkou "Kde dostanem lepšiu hypotéku?". Vhodnejšou otázkou je v tomto prípade otázka "Ako som sa pripravil na získanie hypotéky?". Ideálne je, mať ucelenú predstavu financovania už dlhší čas dopredu a na realizáciu sa dôsledne pripraviť, mať plán. Úspora medzi naplánovaným financovaním a spontánnym rozhodnutím vyriešiť to "zo dňa na deň" môže dosiahnuť až 30%. Pri financovaní vo výške 100.000 EUR je 30% z toho určite zaujímavá suma, súhlasíte? Som presvedčený, že viete ušetrené peniaze využiť pre vás prospešnejšie než "dotovaním banky".</p>
                    </div>
                <img src={img1} alt="" className="boxImg--right"/>
            </div>
                
            <div className="box">
                <img src={img2} alt="" className="boxImg--left"/>
                <div className="boxText">
                    <h1>Druhé a ďalšie bývanie</h1>
                    <hr className="hr--podNadpisom" />
                    <p>Časom sa naše možnosti, ale aj preferencie menia. Preto ľudia s vlastným bývaním sa časom obzerajú po zmene. S rozrastajúcou sa rodinou sa obzeráme po väčšej nehnuteľnosti s vyšším komfortom, vyhovujúcejšou lokalitou. A možno už len chcete mať konečne pokoj od svokrovcov :) Tu často dostávam otázku "Kde dostanem najnižší úrok?" Zďaleka to nie je najdôležitejší parameter. Vhodnejšie je si klásť otázku: "Ako znížiť preplatenosť?" Mimoriadne splátky a čo najkratšia doba splatnosti nie je vždy najlepším riešením.</p>
                    <p>Aj v tomto prípade platí, že správne nastavený plán je kľúčom k efektívnejšiemu využitiu Vašich neľahko zarobených peňazí.</p>
                </div>
            </div>

            <hr className="horizontalLine" />
{/* segment ------------------------------------------------*/}
            <div className="uvod">
                <div className="uvod--nadpis">
                    <h1>RENTA ALEBO LEN DÔCHODOK</h1>
                </div>
                <div className="uvod--text">
                    <p>Dôchodok je veľmi často komunikovanou avšak nie veľmi jasnou témou. Zamysleli ste sa niekedy nad tým, ako bude vyzerať konkrétne vo vašom prípade? Myslíte si, že zastropovanie odchodu do dôchodku je správnym rozhodnutím? Viete si vypočítať aký dôchodok Vás čaká po skončení aktívnej práce? Skrývať sa za odpoveď "Nedožijem sa", nie je zodpovedné, pretože väčšina z nás do dôchodkového veku príde. Nehovoriac o tom, že priemerná dĺžka života na dôchodku je 20 rokov a neustále sa zvyšuje. A čo potom?! Potom už nič, bude neskoro. Na čo sa treba pripraviť a akým spôsobom? Rád Vám zodpoviem Vaše otázky.</p>
                </div>
            </div>
            
            <div className="box">
                    <div className="boxText">
                        <h1>Môžem ísť aj skôr</h1>
                        <hr className="hr--podNadpisom" />
                        <p>Vo všeobecnosti platí, že človek môže ísť do dôchodku, keď na to má dostatočnú finančnú rezervu prípadne iné aktíva. Byť 40-ročný rentier v dnešnej dobe, nie je nič nereálne, a pritom nemusí ísť o dcéru/syna z bohatej rodiny alebo niekoho, kto nadobudol majetok nelegálnym spôsobom. Taký je žiaľ všeobecný názor. Správnymi rozhodnutiami sa dá majetok vybudovať legálne a aj bez nutnosti veľkých finančných investícií.</p>
                    </div>
                <img src={img3} alt="" className="boxImg--right"/>
            </div>

            <div className="box">
                <img src={img4} alt="" className="boxImg--left"/>
                <div className="boxText">
                    <h1>Spotrebiteľský šok</h1>
                    <hr className="hr--podNadpisom" />
                    <p>Štatisticky je dané, že väčšina ľudí pracuje až do momentu naplnenia dôchodkového veku.</p>
                    <p>Každý z nás si jeseň života predstavujeme inak. Čo je však pre všetkých rovnaké, je väčšie množstvo voľného času, ktorý dáva okrem iného aj väčší priestor zvýšeniu osobnej spotreby na tovary a služby. Na druhej strane dochádza k zníženiu príjmu do domácnosti, a to výraznou čiastkou. Ten kto nemá aj ďalší zdroj príjmu okrem 1.-ho pripadne 2.-ho dôchodkového piliera, zažije takzvaný "spotrebiteľský šok". Čaká ho pokles príjmu o cca 40%-50%, čo ale neznamená, že budeme o 40% menej jesť, spotrebovávať elektriku o 40% menej, atď. Kde zobrať zvyšok peňazí na bezproblémový chod domácnosti a zároveň aktívne užívanie si dôchodku?</p>
                </div>
            </div>

            <hr className="horizontalLine" />
{/* segment ------------------------------------------------*/}
            <div className="uvod">
                <div className="uvod--nadpis">
                    <h1>ZABEZPEČENIE ZDRAVIA A MAJETKU</h1>
                </div>
                <div className="uvod--text">
                    <p>Život nám prináša rôzne situácie. Súčasťou sú chvíle šťastia, spokojnosti, víťazné momenty. Tie sú však striedané neplánovanými momentmi, v ktorých nás život skúša. Skúšky spravidla prichádzajú nečakane a v rôznych formách.</p>
                </div>
            </div>
            
            <div className="box">
                    <div className="boxText">
                        <h1>Zabezpečenie príjmu</h1>
                        <hr className="hr--podNadpisom" />
                        <p>Snáď jednou z najťažších skúšok je podlomené zdravie. V tom období peniaze nie sú prioritou. Ich nedostatok však so sebou prináša problémy v ďalších oblastiach života. Veď viete si predstaviť pokles príjmu do domácnosti z mesiaca na mesiac o 50-100%? Na druhej strane dostatok peňazí je predpokladom rýchlejšej a kvalitnejšej liečby. Akým spôsobom ste zabezpečení pre tieto prípady? Najefektívnejšou formou je kvalitné poistné krytie. Ak ho už využívate, dôležité sú správne nastavené parametre. V prípade záujmu Vám s tým rád pomôžem.</p>
                    </div>
                <img src={img5} alt="" className="boxImg--right"/>
            </div>
                
            <div className="box">
                <img src={img6} alt="" className="boxImg--left"/>
                <div className="boxText">
                    <h1>Zabezpečenie majetku</h1>
                    <hr className="hr--podNadpisom" />
                    <p>Na jednej strane si každý z nás budujeme nejakou formou svoj majetok, najčastejšie je to práve bývanie, na druhej strane môže byť náš majetok behom chvíle čiastočne poškodený, alebo úplne zničený. Cena za poistné krytie majetku je zanedbateľná oproti energii, času a peniazom, ktoré sme do vybudovania investovali. Využívate aspoň tie najzákladnejšie krytia, ktoré vedia kompenzovať vzniknuté finančné straty? Či spoliehate sa na šťastie? Rovnako aj tu sú dôležité správne nastavené parametre.</p>
                </div>
            </div>

            <hr className="horizontalLine" />
{/* segment ------------------------------------------------*/}
            <div className="uvod">
                <div className="uvod--nadpis">
                    <h1>TVORBA REZERV</h1>
                </div>
                <div className="uvod--text">
                    <p>Investovať do budúcnosti detí sa oplatí. Bez ohľadu na to, či nám rodičia finančne pomohli / nepomohli v čase štúdia, alebo období stavania sa na vlastné nohy, iste sa zhodneme na tom, že ide o veľkú pomoc. Zrejme každý rodič si želá pre svoje deti to najlepšie. Aj v tomto prípade je to však otázka peňazí. Ako sa na to pripraviť?</p>
                </div>
            </div>
            
            <div className="box">
                    <div className="boxText">
                        <h1>Investovanie</h1>
                        <hr className="hr--podNadpisom" />
                        <p>Koľko to stojí a kde na to vziať? Napriek tomu, že v každom z prípadov ide o nemalý obnos peňazí, pravidelným a disciplinovaným odkladaním aj zanedbateľnej čiastky z výplaty vie každý z nás vytvoriť potrebnú sumu ako rezervu pre deti. Okrem pravidelnosti a disciplíny je dôležitý výber správneho nástroja na finančnom trhu a optimálne nastaviť jeho parametre. Ak hľadáte riešenie, rád Vám pomôžem.</p>
                    </div>
                <img src={img7} alt="" className="boxImg--right"/>
            </div>
                
            <div className="box">
                <img src={img8} alt="" className="boxImg--left"/>
                <div className="boxText">
                    <h1>Deti</h1>
                    <hr className="hr--podNadpisom" />
                    <p>Premýšľali ste niekedy nad tým, ako v budúcnosti pomôcť svojim deťom? Chcete pre ne kvalitné vzdelanie? Alebo poskytnúť im možnosť cestovať už počas štúdia a umožniť tak spoznávať svet? Prípadne dať "štartovné" do života na získanie vlastného bývania? Toto sú najčastejšie plány pre deti, s ktorými sa stretávam. Máte iné? Podeľte sa o ne.</p>
                </div>
            </div>


{/* segment ------------------------------------------------*/}



        </div>
    )
    
}